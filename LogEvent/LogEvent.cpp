// LogEvent.cpp : Defines the entry point for the console application.
//
...
void install_event_log_source(const std::string& a_name)
{
    ...
    const std::string key_path("SYSTEM\\CurrentControlSet\\Services\\EventLog\\" + a_name);
    ...
    {
        BYTE exe_path[] = "d:\\Training\\Tools\\Debug\\LogEvent.exe";  //"C:\\path\\to\\your\\application.exe";
        DWORD last_error;
...
{
    const std::string event_log_source_name("TestLog");
